...
if sprache == "Deutsch":
    ...
    with col2:
        jahreszinssatz = st.number_input('Zinssatz des Darlehens (%)', format='%.2f', placeholder= "", value = float(5.00), step = float(0.10))
    with col3:
        laufzeit_in_jahren = st.number_input('Laufzeit des Darlehens in Jahren', format='%.0f', placeholder= "", value = float(30), step = float(1), min_value=float(0),max_value=float(30))
    ...
    with spar2:
        kredit2_laufzeit = st.number_input('Laufzeit Bauspardarlehen in Jahren', format='%.0f', placeholder= "", value = float(20), step = float(1), min_value=float(0),max_value=float(20))
    with spar3:
    ...
    if rentenposition >= 15000:
        bausparen = rentenposition * faktor
    else:
        bausparen = "Die Position in der Zusatzrente ist zu niedrig für ein Bauspardarlehen (mindestens 15.000€)"

    if isinstance(bausparen, float):
        if bausparen > 150000:
            bausparen = format_eu(150000)
            check = 1
        else:
            bausparen = format_eu(bausparen)
            check = 1
    else:
        bausparen = bausparen
        check = 0
    ...
    if check == 1:
        st.markdown(f"""Die Summe für Ihr Bauspardarlehen beträgt: **{bausparen}€**""")
    elif check == 0:
        st.markdown(f"""{bausparen}""")
    ...
    with col2:
        jahreszinssatz = st.number_input('Tasso di interesse del prestito (%)', format='%.2f', placeholder= "", value = float(5.00), step = float(0.10))
    with col3:
    ...
    if rentenposition >= 15000:
        ...
        bausparen = "La posizione nel fondo pensione complementare è troppo bassa (minimo 15.000€)"
    ...
    if check == 1:
        st.markdown(f"""La somma del Mutuo Risparmio Casa ammonta a: **{bausparen}€**""")
    elif check == 0:
        st.markdown(f"""{bausparen}""")
